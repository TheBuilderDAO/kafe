...
export const TutorialPrepublishCommand = () => {
  ...
  const log = (object: any) =>
    _log(object, tutorialPrepublish.optsWithGlobals().key);
  ...
  tutorialPrepublish
    ...
    .addOption(
      new commander.Option(
        '--force',
        'Force Rewrite the lock file base on slug',
      ).default(false),
    )
    .addOption(
    ...
    .action(async (learnPackageName, options) => {
      ...
      if (options.force) {
        const proposal = await client.getTutorialBySlug(
          lock.chain.get('slug').value(),
        );
