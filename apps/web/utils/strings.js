export const cleanupMarkdownHeading = heading => {
...
export const cleanupStringFromMarkdown = string => {
  string = string.replace(/^(-\s*?|\*\s*?|_\s*?){3,}\s*$/gm, '');
  return string;
};

export const stringToSlug = (s, opt = {}) => {
  s = String(s);
  opt = Object(opt);
...
export const addEllipsis = (text, charCount = 4) => {
...
export const truncateString = (str, n = 120) => {
  if (str.length > n) {
