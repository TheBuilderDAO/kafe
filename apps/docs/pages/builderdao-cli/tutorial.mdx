...
2. In order to use some of the commands, you must be in the root of the tutorial folder. (`init`, `prepublish` and `publish` commands are available in the root of the tutorial folder.) You can also run the commands in a subfolder of the tutorial folder. (`prepublish`, `publish`) some commands can run anywhere.(`get`, `list`)
