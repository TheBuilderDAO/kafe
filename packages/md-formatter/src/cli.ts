...
const destinationDir = path.resolve(
  '/Users/necmttn/Projects/crypto/kafe/kafe/tutorials',
);
...
const tutorials = path.join(
  '/Users/necmttn/Projects/crypto/kafe/learn-tutorials',
);

type FolderMeta = { slug: string; files: string[] };
let processCount = 0;
const processQueue = async.queue(
  async (task: { tutorial: TutorialExtended; proposal: SolanaDetailsRow }) => {
    ...
    processCount++;
    console.log(
      `totalProcessed: [${processCount}] inQueue: [${processQueue.length()}]`,
    );
    await sleep(2000);
...
const getFiles = async (dir: string, level = 0): Promise<any> => {
  return fs.readdir(dir).then(async (fileOrDirs: string[]) => {
    const filtered = fileOrDirs
      .filter(file => !file.startsWith('.'))
      .filter(file => !['assets'].includes(file));
    for (const fileOrDir of filtered) {
      ...
      if (f.endsWith('.md')) {
        if (level === 1) {
          ...
          const multiPageTutorialRoot = path
...
async function main(
  task: { tutorial: TutorialExtended; proposal: SolanaDetailsRow },
    ...
    for (const page of task.tutorial.pages) {
      ...
      const file = await unified()
        ...
        .use(remarkCopyLinkedFiles, {
          ...
          fileNamePrefix: `${task.tutorial.slug}${
            task.tutorial.is_multi_page ? `-${page.slug}` : ''
          }-`,
        })
...
${
  page.next_slug
    ? `next:
  title: '${
    _.find(task.tutorial.pages, { slug: page.next_slug })?.title || 'Next'
  }'
  slug: '/${page.next_slug}'
`
    : ''
...
${
  page.previous_slug
    ? `prev:
  title: '${
    _.find(task.tutorial.pages, { slug: page.previous_slug })?.title ||
    'Previous'
  }'
...
---`;
          await template.addContent(
            page.page_number === 1 ? 'index.mdx' : page.slug,
            [frontMatter, String(file)].join('\n'),
          );
        });
    }
...
const showDiff = (source: string, target: string) => {
  diffLines(source, target).forEach(part => {
